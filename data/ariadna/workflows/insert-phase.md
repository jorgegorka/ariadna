...
Example: `/ariadna:insert-phase 72 Fix critical auth bug`
-> after = 72
...
Usage: /ariadna:insert-phase <after> <description>
Example: /ariadna:insert-phase 72 Fix critical auth bug
```
...
```bash
INIT=$(ariadna-tools init phase-op "${after_phase}")
```
...
**Delegate the phase insertion to ariadna-tools:**

```bash
RESULT=$(ariadna-tools phase insert "${after_phase}" "${description}")
```
...
`/ariadna:plan-phase {decimal_phase}`
...
- Don't use this for planned work at end of milestone (use /ariadna:add-phase)
- Don't insert before Phase 1 (decimal 0.1 makes no sense)
- Don't renumber existing phases
- Don't modify the target phase content
- Don't create plans yet (that's /ariadna:plan-phase)
...
- [ ] `ariadna-tools phase insert` executed successfully
