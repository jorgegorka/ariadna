...
**Note:** MILESTONES.md entry is now created automatically by `ariadna-tools milestone complete` in the archive_milestone step. The entry includes version, date, phase/plan/task counts, and accomplishments extracted from SUMMARY.md files.
...
**Delegate archival to ariadna-tools:**

```bash
ARCHIVE=$(ariadna-tools milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
- [ ] User knows next step (/ariadna:new-milestone)
